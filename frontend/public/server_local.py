...
async def run_full_pipeline_api(request: PipelineRequest):
    ...
    # 7. PDF 생성
    output_dir = "C:/GVIC/backend/data/reports"
    ...
    # public 폴더에 복사
    public_pdf = f"C:/GVIC/frontend/public/GVIC_Report_Latest.pdf"
    import shutil
...
async def list_pipeline_reports():
    """생성된 PDF 리포트 목록"""
    report_dir = "C:/GVIC/backend/data/reports"
    reports = []
...
async def direct_download_report(filename: str):
    ...
    file_path = f"C:/GVIC/backend/data/reports/{filename}"
    
    # public 폴더에서도 확인
    if not os.path.exists(file_path):
        file_path = f"C:/GVIC/frontend/public/{filename}"
...
async def analyze_url(request: URLAnalysisRequest):
    ...
    import shutil
    public_pdf_latest = f"C:/GVIC/frontend/public/GVIC_Report_Latest.pdf"
    public_pdf_named = f"C:/GVIC/frontend/public/{os.path.basename(pdf_path)}"
